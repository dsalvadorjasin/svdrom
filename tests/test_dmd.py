...
class BaseTestOptDMD:
    ...
    def test_generate_forecast_time_vector(
        ...
        """
        model = self.optdmd
        assert model._time_fit is not None
        assert model._t_fit is not None
        if dt is not None:
            expected_delta_t: Any = 2
            expected_delta_time: Any = np.timedelta64(2, "s")
            expected_len: float = 10
        else:
            expected_delta_time = np.mean(np.diff(model._time_fit))
            expected_delta_t = np.mean(np.diff(model._t_fit))
            expected_len = 20 / expected_delta_t
        t_forecast, time_forecast = model._generate_forecast_time_vector(
            forecast_span, dt
        )
        ...
        assert t_forecast[0] == model._t_fit[-1] + expected_delta_t, (
            "Expected 't_forecast[0]' to be ahead of t_fit[-1] "
            f"by a value of {expected_delta_t},"
            f"but got a value of {t_forecast[0] - model._t_fit[-1]} instead."
        )
        if np.issubdtype(self.t.dtype, float):
            if not self.hankel_preprocessing:
                assert model.time_fit is not None
                assert time_forecast[0] == model.time_fit[-1] + expected_delta_t, (
                    "Expected 'time_forecast[0]' to be ahead of time_fit[-1] "
                    f"by a value of {expected_delta_t} "
                    f"but got {time_forecast[0] - model._time_fit[-1]} instead."
                )
            else:
                assert model.time_fit_original is not None
                assert (
                    time_forecast[0] == model.time_fit_original[-1] + expected_delta_t
                ), (
            ...
            if not self.hankel_preprocessing:
                assert model.time_fit is not None
                assert time_forecast[0] == model.time_fit[-1] + expected_delta_time, (
                    "Expected 'time_forecast[0]' to be ahead of time_fit[-1] "
                ...
                assert (
                    time_forecast[0]
                    == model.time_fit_original[-1] + expected_delta_time
                ), (
                    "Expected 'time_forecast[0]' to be ahead of time_fit_original[-1] "
                    f"by a value of {expected_delta_time} "
                    f"but got {time_forecast[0] - model._time_fit[-1]} instead."
    ...
    def test_predict(self, solver: str) -> None:
        ...
        if model.num_trials == 0:
            ...
            # no Dask
            forecast_np, forecast_var_np = model._predict(t, use_dask=False)
            ...
            # with Dask
            forecast_da, forecast_var_da = model._predict(t, use_dask=True)
            assert isinstance(forecast_da, da.Array), (
    ...
    def test_forecast(self, solver: str) -> None:
        ...
        if model.num_trials == 0:
            ...
            # with bagging
            forecast, forecast_var = model.forecast(forecast_span=forecast_span, dt=dt)
            assert isinstance(forecast, xr.DataArray), (
                "Expected 'forecast' to be of type 'xr.DataArray', "
                f"but got {type(forecast)} instead."
            )
    ...
    def test_generate_reconstruct_time_vector(self, t: slice | int) -> None:
        ...
        model = self.optdmd
        assert model._t_fit is not None
        t_reconstruct, time_reconstruct, _ = model._generate_reconstruct_time_vector(t)
        expected_t_reconstruct = np.atleast_1d(model._t_fit[t])
        if not self.hankel_preprocessing:
            assert model._time_fit is not None
            expected_time_reconstruct = np.atleast_1d(model._time_fit[t])
        else:
            assert model._time_fit_original is not None
            expected_time_reconstruct = np.atleast_1d(model._time_fit_original[t])
            if isinstance(t, slice):
                expected_t_reconstruct = expected_t_reconstruct[: -self.d + 1]
            else:
                expected_t_reconstruct = model._t_fit[t - self.d + 1]
    ...
    def test_reconstruct(self, solver: str, t: slice | int) -> None:
            ...
            if isinstance(t, slice):
                if self.hankel_preprocessing:
                    expected_reconstruction_shape = (
                        model._modes.shape[0] // self.d,
                        5,
                    )  # 5: len of slice
                else:
                    expected_reconstruction_shape = (
                        model._modes.shape[0],
                        5,
                    )  # 5: len of slice
        ...
        if model.num_trials == 0:
            ...
            for array in (reconstruction_mean, reconstruction_var):
                ...
                assert np.array_equal(
                    array[model.time_dimension].values, model.time_fit[t]
                ), (
                    "Expected the reconstruction time vector to be: "
                    f"{model.time_fit[t]}, "
...
class TestOptDMDHankelMatrix(TestOptDMDCoherentSignal):
    ...
    def test_reconstruct_vs_pydmd(self) -> None:
        ...
        reconstruction_da = self.optdmd.reconstruct()
        assert isinstance(reconstruction_da, xr.DataArray)
        reconstruction = reconstruction_da.values.real
