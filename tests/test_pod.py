...
def test_compute_methods() -> None:
    ...
    lazy_modes, lazy_time_coeffs = pod.modes, pod.time_coeffs
    lazy_energy_ratio = pod.explained_energy_ratio
    assert lazy_modes is not None
    assert lazy_time_coeffs is not None
    assert isinstance(lazy_modes.data, da.Array)
    assert isinstance(lazy_time_coeffs.data, da.Array)
    assert isinstance(lazy_energy_ratio, da.Array)
